...
//
// Dev credentials (LOCAL ONLY — never real): login dev@smplmark.test / smplmark-dev-password.
...
--   Web login:  dev@smplmark.test  /  ${DEV_PASSWORD}
--
...
INSERT INTO user (id, email, email_verified, display_name, created_at) VALUES
  (${q(userId)}, 'dev@smplmark.test', 1, 'smplmark dev', ${T0});
...
INSERT INTO account (id, key, name, description, created_at, allow_personal_publish) VALUES
  ('acct-system', 'system', 'smplmark', 'Openly licensed benchmark results ingested from third-party sources. Every ingested benchmark credits its source and license, and links back to the original data.', ${T0}, 0);

-- The dev workspace: personal publish pre-enabled so the whole publish flow is exercisable.
INSERT INTO account (id, key, name, description, created_at, allow_personal_publish) VALUES
  (${q(acctId)}, 'dev', 'Dev Publisher', 'A local development account for trying the console end to end.', ${T0}, 1);
