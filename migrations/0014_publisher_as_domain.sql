...
-- user can't verify a throwaway domain and then impersonate a well-known company with any name+logo they like.
-- Replace publisher_identity (1) + publisher_domain (N) with a single `publisher` table that IS a
-- domain. Frozen ORG attribution snapshots on already-published benchmarks are not migrated (nothing depended
-- on them at this point). Drop the child table first (FK order).
