...
/** Resolve an account's subject by its human key (unique per account, per the subject_account_key index). */
...
export async function deleteSubjectCascade(db: D1Database, id: string): Promise<void> {
  await db.batch([
    ...
    // Runs across every linked benchmark survive this delete — shed the removed rows from their
    // denormalized measurement_count BEFORE the delete (the correlated count reads live rows).
    db
      .prepare(
        "UPDATE run SET measurement_count = MAX(measurement_count - (SELECT COUNT(*) FROM measurement WHERE measurement.run_id = run.id AND measurement.subject_id = ?), 0) WHERE id IN (SELECT DISTINCT run_id FROM measurement WHERE subject_id = ?)",
      )
      .bind(id, id),
