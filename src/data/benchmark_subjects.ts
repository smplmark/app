 ...
 * subtree is frozen, so the subject's own mutations must be refused too — a subject is shared, so
...
export async function deleteBenchmarkSubjectCascade(
  ...
  await db.batch([
    // The benchmark's runs survive this delete, so their denormalized measurement_count must shed the
    // removed rows. Decrement BEFORE the delete (the correlated count reads the still-live rows).
    db
      .prepare(
        "UPDATE run SET measurement_count = MAX(measurement_count - (SELECT COUNT(*) FROM measurement WHERE measurement.run_id = run.id AND measurement.subject_id = ?), 0) WHERE benchmark_id = ?",
      )
