...
// carries a customer-facing description: no storage-mechanic language, no internal keys.
//
...
// The derived-metric formula vocabulary. Defined here (ahead of the measurement_schema value objects
// that embed it) because both a benchmark's `measurement_schema.derived[]` and the standalone `metric`
// resource surface the same formula shape.
const MetricToken = z
  .object({
...
const MetricDecl = z
  .object({
    ...
    label: z.string().optional().openapi({ description: "The metric's display name, as the publisher wrote it. Absent when the benchmark declares the metric without a library definition behind it." }),
...
const DerivedDecl = z
  .object({
    ...
    label: z.string().optional().openapi({ description: "The metric's display name, as the publisher wrote it." }),
    ...
    formula: MetricFormula.optional().openapi({ description: "The formula that produces the value, as an ordered list of lettered steps. Resolved from the benchmark's linked library metric on every read, so it always reflects the definition in force now." }),
    expr: z.record(z.unknown()).optional().openapi({ description: "The JSON Logic (https://jsonlogic.com/) that `formula` compiles to — the expression evaluated against each measurement to produce the value. `{ \"var\": \"metrics.<name>\" }` reads another metric on the same measurement; `{ \"var\": \"created_at\" }` reads its creation time in epoch milliseconds.", type: "object" }),
  })
  .openapi("DerivedDecl", { description: "A metric computed on read from the benchmark's linked library metric; its value appears in each measurement's `metrics` map. The formula that computes it is surfaced here in both forms — structured steps (`formula`) and the JSON Logic they compile to (`expr`) — so anyone reading the benchmark can see exactly how the number is produced." });
...
const metaCountsParam = registry.registerParameter(
  "MetaCounts",
  z.string().optional().openapi({
    param: { name: "meta[counts]", in: "query" },
    description:
      "Set to 'true' to include per-subject measurement counts in the response meta, computed over the full filtered set (all matching measurements, not just the returned page).",
  }),
);
...
registry.registerPath({
  ...
  description:
    "Permanently deletes a private benchmark and its runs and measurements. A published (or withdrawn) benchmark can never be deleted this way — the public record must not vanish; withdraw it instead, or report an issue for removal by smplmark operators.",
  security: bearerSecurity,
  request: { params: benchmarkIdParam },
  responses: {
    "204": { description: "The benchmark was deleted." },
    ...commonErrors,
    "409": errorJson("The benchmark is published or withdrawn (withdraw or report an issue instead), or it is marked ready for publishing."),
  },
});
...
registry.registerPath({
  ...
  responses: {
    ...
    ...commonErrors,
    "429": errorJson("Rate limit exceeded."),
    "409": errorJson("The run and subject belong to different benchmarks, the benchmark is closed, or the run has reached its 100,000-measurement capacity."),
  },
});
...
const MeasurementStats = registry.register(
  ...
  z
    .object({
      ...
      truncated: z
        .boolean()
        .openapi({ description: "True when the set exceeded the summary scan limit, so the figures cover only the most recent measurements — narrow the timeframe for exact values." }),
...
const SubjectMeasurementCount = registry.register(
  "SubjectMeasurementCount",
  z
    .object({
      subject: z.string().openapi({ description: "The subject's id." }),
      count: z.number().int().openapi({ description: "Measurements recorded for this subject in the filtered set." }),
    })
    .openapi({ description: "One subject's measurement count within the filtered set." }),
);
    ...
    .object({
      ...
      meta: z
        .object({
          ...
          counts: z.array(SubjectMeasurementCount).optional().openapi({
            description: "Present only when meta[counts]=true was requested. One entry per subject with measurements in the set, sorted by subject id.",
          }),
          stats: MeasurementStats.optional().openapi({
            description: "Present only when meta[stats]=true was requested.",
          }),
        })
        .openapi({ description: "Pagination details, plus per-subject counts and aggregate statistics when requested." }),
    })
    .openapi({ description: "A page of measurements, with per-subject counts in the meta when meta[counts]=true and aggregate statistics when meta[stats]=true." }),
);
...
registry.registerPath({
  ...
  description:
    "Reads measurements for exactly one of a run, subject, or benchmark. With an Accept header of text/csv, the response is a CSV export of the same data. Pass meta[counts]=true for per-subject measurement counts, or meta[stats]=true for aggregate statistics (per subject and metric), both computed over the full filtered set and returned in the response meta.",
  parameters: [
    filterParam("run", "Read measurements for this run id. Provide exactly one of filter[run], filter[subject], or filter[benchmark] — except filter[run] may be combined with filter[benchmark], which resolves the run id within that benchmark (run ids are unique per benchmark, not globally)."),
    ...
    filterParam("benchmark", "Read measurements for this benchmark id. May be combined with filter[run] (see filter[run]); otherwise provide exactly one scope filter."),
    filterParam(
      "created_at",
    ...
    ...paginationParams,
    { $ref: "#/components/parameters/MetaCounts" },
  ...
  responses: {
    ...
    "400": errorJson("The query parameters were malformed, no resource filter was provided, or filter[subject] was combined with another scope filter."),
...
const HistoryActor = z
  .object({
    type: z
      .string()
      .nullable()
      .openapi({ description: "Who acted: USER, API_KEY, OPERATOR, PUBLIC (an anonymous visitor, e.g. someone who reported an issue), or — on redacted views — PUBLISHER (individual identities are never shown publicly)." }),
    id: z
      .string()
      .nullable()
...
const HistoryEventAttributes = registry.register(
  ...
  z
    .object({
      ...
      resource_type: z
        .string()
        .openapi({ description: "The kind of resource the event is about: benchmark, run, measurement, subject, or issue." }),
...
const issueAttributes = z
  .object({
    benchmark: idRef("The published benchmark the issue is about."),
    requester_name: z.string().openapi({ description: "The reporter's name." }),
    requester_email: z.string().openapi({ description: "The reporter's email address, used to follow up on the issue." }),
  ...
  .openapi({ description: "An issue reported so smplmark operators can review and remove a published benchmark." });

const IssueResponseSchema = registry.register(
  "IssueResponse",
  z
    .object({
      data: z
        .object({
          id: z.string().openapi({ description: "The unique identifier of the issue." }),
          type: z.literal("issue").openapi({ description: "Always \"issue\"." }),
          attributes: registry.register(
            "Issue",
            issueAttributes.extend({
              status: z.string().openapi({ description: "The issue's state: OPEN until operators resolve it." }),
              created_at: dateTime("When the issue was reported."),
            }),
          ),
        })
        .openapi({ description: "A single issue resource object." }),
    })
    .openapi({ description: "A response wrapping the reported issue." }),
);

const IssueRequestSchema = registry.register(
  "IssueRequest",
  z
    .object({
      data: z
        ...
        .openapi({ description: "The issue to report." }),
    })
    .openapi({ description: "A request body carrying an issue to report." }),
);
...
registry.registerPath({
  ...
  path: "/api/v1/issues",
  tags: ["Issues"],
  summary: "Report an issue with a published benchmark",
  description:
    "Reports an issue asking smplmark operators to remove a published benchmark (for example, one containing personal data or infringing content). This does not change the benchmark: operators review each issue and perform the removal themselves. Unauthenticated and rate-limited; the reporter's identity is shared only with operators, never published.",
  request: { body: domainBody(IssueRequestSchema, "The issue to report.") },
  responses: {
    "201": domainResponse(IssueResponseSchema, "The reported issue."),
    "400": errorJson("The request was malformed."),
    "404": errorJson("The requested resource was not found."),
    "429": errorJson("Rate limit exceeded."),
  },
