...
registry.registerPath({
  ...
  description:
    "A server-sorted, server-filtered page of a benchmark's targets, each with its representative (latest) observation's metrics — the read behind the viewer's large-benchmark mode. Sort by any metric declared in the benchmark's observation schema (default: the chart metric, descending). Narrow with filter[search] (free-text over target name and details) and filter[facet.<field>] (an exact match on a details field; repeat the parameter to OR several values, e.g. filter[facet.vendor]=AMD&filter[facet.vendor]=Intel). meta.facets lists each facetable details field with value counts over the current filter. Request `Accept: text/csv` (or add `format=json`) to download the entire current filter as a file instead of a page — up to the benchmark's target limit, without facets.",
  parameters: [
