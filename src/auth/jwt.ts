// Session JWTs — the web-login credential. HS256 via jose, verified statelessly (signature +
// exp + iss + aud). The `session` table records issuance for audit/logout; the hot path never reads
// it. Claims: iss=APP_URL, aud=smplmark, sub=user_id, plus account_id/role.
