// Publishers — a publisher IS a domain, TXT-verified via DNS. Writes are admin-gated and need an
...
publishers.post("/", requireAuth, rateLimitByCredential((e) => e.RL_WRITE), async (c) => {
  const auth = getAuth(c);
  requireAccountScope(auth);
  requireAdmin(auth);
