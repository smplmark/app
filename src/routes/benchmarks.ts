...
import { loadLiveDerivedByBenchmark, loadMetricLabelsByBenchmark } from "../logic/live_derived";
import {
  CATEGORIES,
  type AuthContext,
  type BenchmarkRow,
...
/** filter[category]: SCREAMING_SNAKE_CASE enum, case-insensitive on input. */
...
/** The draft/ready flag transitions: the author (a writer) or any admin. */
...
benchmarks.post("/", requireAuth, rateLimitByCredential((e) => e.RL_WRITE), async (c) => {
  const auth = getAuth(c);
...
benchmarks.get("/", optionalAuth, async (c) => {
  ...
  // …and each linked metric's display label, which the schema snapshot doesn't carry.
  const labelMap = await loadMetricLabelsByBenchmark(
    c.env.DB,
    rows.map((r) => r.id),
  );
  return collectionResponse(
    rows.map((r) =>
      serializeBenchmark(r, tagsByBenchmark.get(r.id) ?? [], liveMap.get(r.id), labelMap.get(r.id)),
    ),
...
benchmarks.get("/:id", optionalAuth, async (c) => {
  ...
  // Live derived (from the current library metric) replaces the stored snapshot in the response, and
  // each linked metric's display label is folded in.
  const live = (await loadLiveDerivedByBenchmark(c.env.DB, [row.id])).get(row.id);
  const labels = (await loadMetricLabelsByBenchmark(c.env.DB, [row.id])).get(row.id);
  return resourceResponse(
    serializeBenchmark(row, await listTagsForBenchmark(c.env.DB, row.id), live, labels),
  );
});
...
// action (see routes/jobs.ts), reachable by reporting an issue.
