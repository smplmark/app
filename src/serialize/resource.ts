...
// SINGULAR `type`, NO relationships (parent refs are bare
...
export function serializeBenchmark(
  ...
  // The display label of each linked library metric, keyed by metric name (loadMetricLabelsByBenchmark).
  // Folded into the schema below so a metric reads as the publisher wrote it ("Total Time") rather than
  // as a de-slugged column name. Omitted → no labels are added.
  metricLabels?: Record<string, string>,
): ResourceObject {
  const measurement_schema = parseMeasurementSchema(row.measurement_schema);
  const labelFor = (name: string): string | undefined => metricLabels?.[name];
  measurement_schema.metrics = measurement_schema.metrics.map((m) => {
    const label = labelFor(m.name);
    return label === undefined ? m : { ...m, label };
  });
  // Surface each derived metric WITH its formula — both the structured `formula` (lettered steps) and
  // the JSON Logic `expr` it compiles to. A published benchmark's numbers have to be auditable by the
  // people reading them, so how a derived value is computed is part of the public record, not an
  // implementation detail. Both come from the live library metric (liveDerived); a legacy stored
  // snapshot contributes whatever `expr` it recorded and no structured formula.
  ...
  measurement_schema.derived = derivedSource.map((d) => {
    ...
    const label = d.label ?? labelFor(d.name);
    if (label !== undefined) pub.label = label;
    ...
    if (d.formula !== undefined) pub.formula = d.formula;
    if (d.expr !== undefined) pub.expr = d.expr;
    return pub;
  });
  const attributes: Record<string, unknown> = {
    account: row.account_id,
...
/** An issue as echoed back to its (possibly anonymous) reporter. */
