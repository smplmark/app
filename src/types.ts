...
/**
 * Account membership role. A strict superset chain: each tier inherits everything
...
/**
 * A publisher IS a domain. It's publishable once its domain is VERIFIED via DNS TXT. There's no
...
/** The unary functions a formula step can apply. `created_at mod 60000` expresses a minute offset, so no
 *  dedicated time-offset function is needed — floor/round/ceil/abs plus MOD cover it from primitives. */
...
export interface RunRow {
  ...
  /** Denormalized ingest counter, maintained in the same batch as each measurement insert/delete.
   *  Enforces the per-run ceiling without a COUNT(*) per write; may drift by a few under races. */
  measurement_count: number;
  created_at: number;
  updated_at: number;
}

/** Issue lifecycle: OPEN until an operator resolves (or otherwise closes) it. */
export type IssueStatus = "OPEN" | "RESOLVED";

/**
 * An issue reported against a published benchmark (e.g. legal/PII), filed from the public page or
 * the console and routed to smplmark operators. Never a self-serve delete: an operator acts on it
 * via the system removal endpoint. benchmark_id is a soft pointer (no FK) — the row must survive
 * the benchmark's deletion as the record of why it was removed.
 */
export interface IssueRow {
  id: string;
  benchmark_id: string;
  /** Snapshotted at filing so the issue stays legible after the benchmark is removed. */
...
export interface MetricDecl {
  name: string;
  type: string;
  /** The metric's human-readable display name, resolved on read from the linked library metric.
   *  Cosmetic; absent when no library metric is linked (a hand-written schema). */
  label?: string;
...
export interface DerivedDecl {
  name: string;
  /** The metric's human-readable display name, resolved on read from the linked library metric.
   *  Cosmetic; absent on a legacy stored snapshot. */
  label?: string;
  /** Cosmetic display label. */
  unit?: string;
  /** Excel-style number-format pattern for display. Cosmetic. */
  format?: string;
  /** The compiled JSON Logic formula the compute-on-read engine evaluates — resolved live from the
   *  linked library metric, and surfaced on read so a benchmark's Metrics tab can show visitors
   *  exactly how the value is computed. Optional because a stored snapshot (or a client's
   *  get-mutate-put round-trip) may omit it. */
  expr?: JsonLogicRule;
  /** The structured formula (lettered steps) `expr` compiles from — the readable form rendered
   *  alongside the JSON Logic. Present on a live-resolved decl; a legacy stored snapshot carries
   *  `expr` alone. Never written back to a stored snapshot. */
 ...
 * `scope_ref` are the *effective* authority used by the authorization layer.
 */
