// Compute-on-read: merge an measurement's stored metrics with its benchmark's derived metrics into
// one flat object. The consumer can't tell stored from derived — that split lives only in
// measurement_schema. The evaluation context is WIDENED beyond the measurement's own
 ...
 * (never a 5xx). Returns null when nothing is emitted, so the serializer omits `metrics`.
 */
