// Transactional email via Resend's REST API. Best-effort by contract: a send failure never
...
// ── Issue reported (ops alert to support) ────────────────────────────────────

/** Route a reported issue to the operators. Best-effort: the issue row is already persisted. */
export async function sendIssueReportedEmail(
  ...
  const benchmarkUrl = `https://www.smplmark.org/benchmarks/${input.benchmarkRef}`;
  const rows = [
    ["Benchmark", `${input.benchmarkName} (${benchmarkUrl})`],
    ["Benchmark ID", input.benchmarkId],
    ["Reporter", `${input.requesterName} <${input.requesterEmail}>`],
    ["Issue ID", input.requestId],
  ]
    .map(
      ([k, v]) =>
        `<tr><td style="padding:4px 12px 4px 0;color:${MUTED}">${esc(k)}</td><td style="padding:4px 0">${esc(v)}</td></tr>`,
...
<p style="color:${MUTED};font-size:13px">Removing the benchmark is an operator action (the system benchmark-removal endpoint) — never a self-serve delete. Reply to this email to respond to the reporter.</p>`,
  );
  const text = `Issue reported\n\nBenchmark: ${input.benchmarkName} (${benchmarkUrl})\nBenchmark ID: ${input.benchmarkId}\nReporter: ${input.requesterName} <${input.requesterEmail}>\nIssue ID: ${input.requestId}\n\n${input.reason}`;
  return sendEmail(env, {
    to: SUPPORT_EMAIL,
    subject: `Issue reported: ${input.benchmarkRef}`,
    html,
    text,
