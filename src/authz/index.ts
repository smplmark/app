// Authorization. One check, fed by both credential sources. A credential's *effective* scope
...
/** A resource's ancestry, from the *chains* data helpers. There is intentionally no subject scope —
 *  a measurement's subject is validated to share the run's benchmark, not separately scoped. */
...
/**
 * Authority ceiling for minting keys: a key may not exceed its minter's authority. The
...
// ── Role-based access (VIEWER < MEMBER < ADMIN < OWNER chain) ──
...
/** May create / edit / delete resources (benchmarks, subjects, runs, measurements). */
