...
//    publish / withdraw / removed are the events most worth upgrading to guaranteed delivery
//    (a queue) later.
...
/** An explicitly-named actor for events with no credential (operator removals, public reporters). */
...
export interface AuditEventInput {
  ...
  event_type: string;
  resource_type: "benchmark" | "run" | "measurement" | "subject" | "issue";
  resource_id: string;
  /**
 ...
 * events of that type were written before the cut — dropping them at read time retires the
 * noise without a data migration.
 */
const RETIRED_EVENT_TYPES = new Set(["measurement.created"]);

/** Smpl Audit's interval notation: inclusive bounds, `*` for an open edge. */
