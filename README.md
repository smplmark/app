...
A stranger can sign up, define benchmarks / subjects / runs, upload measurements (by hand or via API),
and publish — no account gets special treatment. smplmark does
...
- **API:** JSON:API (`application/vnd.api+json`), singular resource `type`, plural paths, parent refs
  as bare id attributes (no `relationships`), `snake_case` everywhere.

## Data model

`account (1)→(N) benchmark (1)→(N) run (1)→(N) measurement`; `subject` (the thing being measured)
attaches to benchmarks N↔N via `benchmark_subject`, and each measurement names both its run and its
subject. Metrics are a two-level library: `metric` rows link to benchmarks via `benchmark_metric`.
Plus identity (`user`, `user_identity`, `account_user`, `session`, `email_verification`,
`invitation`) and `api_key`.

- **Roles (per `account_user`):** `VIEWER < MEMBER < ADMIN < OWNER`. VIEWER reads;
  MEMBER edits benchmarks/subjects/runs; ADMIN also manages members, invitations, API keys, and
...
- **Interpretation freeze:** publishing freezes the semantic core of `measurement_schema` (derived
...
**Dev auto-login (no SSO).** With `DEV_LOGIN=1` (uncomment it in `.dev.vars`), just
...
**Dev credentials** (local only): log in at `/login` with `dev@smplmark.test` /
`smplmark-dev-password`. The seed deliberately creates no benchmarks — build one through the
console (create → mark ready → publish → mint a run-scoped key → POST measurements to
`/api/v1/measurements`); that end-to-end flow is the product test drive. Ingested reference data
...
| Group | Endpoints |
| --- | --- |
| Auth (JSON) | `POST /auth/register`, `/auth/login`, `/auth/verify-email`, `/auth/resend-verification`, `/auth/logout`, `/auth/switch`; `GET /auth/oidc/{google\|microsoft}`, `/auth/callback/{provider}` |
| Users / Account | `GET·PUT /users/current`, `GET·PUT /users/current/settings`, `GET /accounts`, `GET·PUT·DELETE /accounts/current`, `GET /accounts/{id}`, `GET /account_users`, `PUT·DELETE /account_users/{userId}` |
| Invitations | `POST·GET /invitations`, `POST /invitations/accept`, `POST /invitations/{id}/actions/{resend\|revoke}` |
| API keys | `POST·GET /api_keys`, `GET·PUT·DELETE /api_keys/{id}` (GET reveals), `POST /api_keys/{id}/actions/{rotate\|revoke}` |
| Benchmarks | `POST·GET /benchmarks`, `GET·PUT·DELETE /benchmarks/{id}`, `GET /benchmarks/{id}/history`, `POST /benchmarks/{id}/actions/{mark_ready\|publish\|withdraw\|close\|reopen\|return_to_draft\|view}` |
| Subjects | `POST·GET /subjects` (`filter[benchmark\|subject_type\|key]`), `GET·PUT·DELETE /subjects/{id}`, `GET /subjects/{id}/history`; `POST·GET·DELETE /benchmark_subjects` (the N↔N link) |
| Subject types | `POST·GET /subject_types`, `GET·PUT·DELETE /subject_types/{id}` |
| Metrics | `POST·GET /metrics`, `GET·PUT·DELETE /metrics/{id}`; `POST·GET·DELETE /benchmark_metrics` (links a metric to a benchmark) |
| Runs | `POST·GET /runs` (`filter[benchmark]` required), `GET·PUT·DELETE /runs/{id}`, `GET /runs/{id}/history`, `POST /runs/{id}/actions/{end\|invalidate}` |
| Measurements | `POST /measurements` (flat; `run` is a required body field), `GET /measurements` (`filter[run\|subject\|benchmark]`; optional `filter[created_at]` range; `Accept: text/csv` for CSV), `PUT·DELETE /measurements/{id}` |
| Publishers | `POST·GET /publishers`, `GET·PUT·DELETE /publishers/{id}`, `POST /publishers/{id}/actions/verify` |
| Misc | `POST /emails`, `POST /issues`, `GET /external_sources` |

`sort` (single field, `-` prefix, per-endpoint default + allowed set), `page[number]`/`page[size]`
(default/cap 1000), and `meta[total]` are honored on read-many endpoints. Mutation is
lifecycle-gated: PRIVATE is a fully-mutable workspace; once PUBLISHED, history is append-only
(invalidation and withdrawal are annotations, never removals — see the status lifecycle above).
...
`migrations/0001_init.sql` is the initial schema; every subsequent schema change is a new
forward-only migration (`0002_*`, …) — append-only forever. `scripts/seed.sql` is generated by
`scripts/gen-seed.mjs` (never edit it by hand).
...
Auth is the `CLOUDFLARE_API_TOKEN` GitHub Actions secret (account id is pinned in
`wrangler.jsonc`).
...
To rebuild from scratch: deploy this Worker (it claims the custom domain), and the secret-sync step
provisions its secrets from the repo secrets above.

## Project layout

```
migrations/0001_init.sql   D1 schema (initial)
...
src/
  ...
  query/ logic/ schema/    range/sort/pagination, json-logic + compute-on-read, measurement_schema
  serialize/               row → JSON:API resource, measurements → CSV
  ...
  routes/                  auth, users, accounts, account_users, invitations, api_keys, benchmarks,
                           subjects, subject_types, benchmark_subjects, metrics, benchmark_metrics,
                           runs, measurements, publishers, issues, emails, external_sources, jobs
```
