...
`main`, gated on typecheck + tests, applying forward D1 migrations first. It needs one repo secret,
`CLOUDFLARE_API_TOKEN` (account id is pinned in `wrangler.jsonc`). Until that secret is set the deploy
job no-ops so CI stays green.

### One-time cutover from the combined Worker

smplmark started as a single Worker (`smplmark`, in the website repo) that served all three custom
domains. This repo splits the app off. The D1 database is unchanged (`smplmark`,
`25d003c3-…`) — only which Worker binds it and serves `app.smplmark.org` changes. Do the cutover in
this order to avoid a custom-domain conflict (Cloudflare won't let two Workers claim the same domain):

1. **Add the secret.** In the `smplmark/app` repo settings, add `CLOUDFLARE_API_TOKEN` (same token the
   website repo uses — it already has D1 + Workers + custom-domain permissions on this account).
2. **Release `app.smplmark.org` from the old Worker.** In the **website** repo, drop the
   `app.smplmark.org` route (and the D1 binding) from its `wrangler.jsonc` and deploy. `www` + apex
   keep working; `app.smplmark.org` is briefly unclaimed. (No customers yet — a short window is fine.)
3. **Deploy this Worker.** Push to `main` here (or re-run the deploy workflow). It applies migrations
   (idempotent — already at head) and claims `app.smplmark.org`.
4. **Verify:** `https://app.smplmark.org/account` (console), `/api/v1/benchmarks` (200 + CORS header
   for the `https://www.smplmark.org` origin), `/api-reference` (Scalar). Then confirm the website's
   published benchmark pages load their data from this API.

Secrets to set on this Worker (`wrangler secret put <NAME>`, or reuse the values already on the old
Worker): `APP_AUTH_SECRET`, `KEY_ENCRYPTION_SECRET`, `APP_URL=https://app.smplmark.org`, plus any
optional OIDC / Resend / `JOBS_TRIGGER_SECRET` values.
