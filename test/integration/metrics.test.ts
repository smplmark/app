// Metrics — CRUD, snake_case name derivation, INTEGER/DECIMAL vs FORMULA + formula → JSON Logic, and authz.
...
describe("metric CRUD + name derivation", () => {
  ...
  it("stores FORMULA metrics and compiles their formulas to JSON Logic", async () => {
    const me = await register();
    // A minute offset is expressible from primitives — `created_at mod 60000` — so no built-in op needed.
    const offsetFormula = {
      steps: [{ id: "A", kind: "OP", op: "MOD", a: { kind: "CREATED_AT" }, b: { kind: "NUMBER", value: 60000 } }],
      result: "A",
    };
    const offset = await create(me.token, { label: "Offset", type: "FORMULA", unit: "ms", formula: offsetFormula });
    expect(offset.attributes).toMatchObject({ type: "FORMULA", unit: "ms" });
    expect(offset.attributes.formula).toEqual(offsetFormula);
    expect(offset.attributes.expr).toEqual({ "%": [{ var: "created_at" }, 60000] });
