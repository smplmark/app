...
describe("benchmark license (publisher-declared)", () => {
  ...
  it("sets and clears the license via the full-replace PUT", async () => {
    ...
    const base = {
      ...
      measurement_schema: OFFSET_SCHEMA,
    };
  ...
  it("rejects a whitespace-only, non-string, or over-long license (400)", async () => {
    ...
    const put = await apiPut(
      ...
      putBody({ name: "Example Bench", subject_type: st, measurement_schema: OFFSET_SCHEMA, license: "\t " }),
      bearer(me.token),
    );
...
describe("publish gate + lifecycle", () => {
  ...
  it("publishes only once a subject, run, and measurement all exist (readiness gate)", async () => {
    const me = await register();
    await markVerified(me.user_id);
    const b = await makeBenchmark(me.token); // OFFSET_SCHEMA satisfies the metric requirement
    await allowPersonalPublish(b.id);
...
describe("post-publish edits (auditable record, not frozen)", () => {
  it("allows cosmetic and semantic-core edits after publish", async () => {
    ...
    const ok = await apiPut(
      `/api/v1/benchmarks/${b.id}`,
      putBody({ name: "Renamed", description: "new tagline", subject_type: st, measurement_schema: OFFSET_SCHEMA }),
      bearer(me.token),
    );
    expect(ok.status).toBe(200);
    ...
    const semantic = await apiPut(
      ...
      putBody({
        ...
        measurement_schema: {
          metrics: [],
          derived: [{ name: "offset_ms", expr: { "+": [1, 1] } }],
          chart: { x: "created_at", y: "offset_ms", x_kind: "TIME" },
        },
      }),
      bearer(me.token),
    );
    ...
    const appended = await apiPut(
      ...
      putBody({
        ...
        measurement_schema: {
          ...OFFSET_SCHEMA,
          derived: [...OFFSET_SCHEMA.derived, { name: "extra_ms", unit: "ms", expr: { "+": [1, 1] } }],
        },
      }),
      bearer(me.token),
    );
    ...
    expect(schema.derived.map((d) => d.name)).toEqual(["offset_ms", "extra_ms"]);
  });
  ...
  it("forbids deleting a published benchmark but allows deleting a private one", async () => {
    ...
    expect(((await del.json()) as { errors: { detail: string }[] }).errors[0].detail).toBe(
      "A published benchmark can't be deleted — the public record must not vanish. Withdraw it instead, or report an issue for removal by smplmark operators.",
    );
  });
});
...
describe("benchmark subject_type (like against like)", () => {
  ...
  it("accepts the subject_type by key or by legacy UUID, on create and update", async () => {
    ...
    const put = (ref: string) =>
      apiPut(
        ...
        putBody({ name: "Renamed", subject_type: ref, measurement_schema: OFFSET_SCHEMA }),
        bearer(me.token),
      );
  ...
  it("rejects an unknown or cross-account subject_type on update (by key or UUID)", async () => {
    ...
    const put = (ref: string) =>
      apiPut(
        `/api/v1/benchmarks/${b.id}`,
        putBody({ name: "R", subject_type: ref, measurement_schema: OFFSET_SCHEMA }),
        bearer(me.token),
      );
  ...
  it("locks subject_type while subjects are linked; editable again once unlinked", async () => {
    ...
    const put = (stx: string) =>
      apiPut(`/api/v1/benchmarks/${b.id}`, putBody({ name: "Example Bench", subject_type: stx, measurement_schema: OFFSET_SCHEMA }), bearer(me.token));
...
describe("tenant isolation", () => {
  it("returns 404 (not 403) when another account touches a private benchmark", async () => {
    ...
    expect(
      (await apiPut(`/api/v1/benchmarks/${bench.id}`, putBody({ name: "x", measurement_schema: OFFSET_SCHEMA }), bearer(b.token))).status,
    ).toBe(404);
