// Issues: the public report affordance (a contact channel, never a delete) and the operator-only
// true delete behind the system endpoint — the only way a published record ever vanishes, and even
// then the removal itself is recorded in Smpl Audit.
...
function issueBody(benchmarkId: string, overrides: Record<string, unknown> = {}) {
  return {
    data: {
      type: "issue",
      attributes: {
...
describe("POST /api/v1/issues (public affordance)", () => {
  it("reports an issue against a published benchmark, emails operators, and records an internal event", async () => {
    ...
    const res = await apiPost("/api/v1/issues", issueBody(bm.id)); // anonymous
    ...
    const row = await env.DB.prepare("SELECT * FROM issue WHERE id = ?").bind(doc.id)
      .first<{ status: string; benchmark_key: string; publisher_slug: string }>();
    expect(row?.status).toBe("OPEN");
    expect(row?.benchmark_key).toBe("example-bench");

    // Routed to operators (support inbox, reply-to the reporter)…
    expect(resendPosts).toHaveLength(1);
    expect(resendPosts[0].to).toBe("support@smplmark.org");
    expect(resendPosts[0].subject).toContain("Issue reported");
    expect(resendPosts[0].reply_to).toBe("requester@example.com");
    // …and recorded internally, never on the public history. The reporter's identity rides in
    // extra (operators only), NEVER in the actor label — the publisher reads the owner-visible
    // History, and the issue is typically filed against them.
    await vi.waitFor(() =>
      expect(auditPosts.filter((e) => e.event_type === "benchmark.issue_reported")).toHaveLength(1),
    );
    const ev = auditPosts[0];
    expect(ev.data.visibility).toBe("internal");
    expect(ev.actor_type).toBe("PUBLIC");
    // Omitted on the wire (the SDK skips unset fields) — the reporter's email must not be here.
  ...
  it("still succeeds when neither email nor audit is configured (best-effort routing)", async () => {
    ...
    const bm = await makeBenchmark(token);
    await publish(token, user_id, bm.id);
    const res = await apiPost("/api/v1/issues", issueBody(bm.id));
    expect(res.status).toBe(201);
    expect(resendPosts).toHaveLength(0);
    expect(auditPosts).toHaveLength(0);
  });

  it("accepts issues against withdrawn benchmarks (they are still public record)", async () => {
    const { token, user_id } = await register();
    const bm = await makeBenchmark(token);
    await publish(token, user_id, bm.id);
    ...
    expect((await apiPost("/api/v1/issues", issueBody(bm.id))).status).toBe(201);
  });

  it("404s for a private or unknown benchmark (no existence leak) and 400s on missing fields", async () => {
    const { token } = await register();
    const bm = await makeBenchmark(token);
    expect((await apiPost("/api/v1/issues", issueBody(bm.id))).status).toBe(404);
    expect((await apiPost("/api/v1/issues", issueBody(crypto.randomUUID()))).status).toBe(404);
    const missing = await apiPost("/api/v1/issues", issueBody(bm.id, { reason: "" }));
    expect(missing.status).toBe(400);
  });
});

describe("POST /api/v1/jobs/benchmark-removal (operator-only true delete)", () => {
  const trigger = (body: unknown, secret?: string) =>
    apiPost("/api/v1/jobs/benchmark-removal", body, secret ? { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" } : { "Content-Type": "application/json" });
  ...
  it("hard-deletes the published benchmark's whole subtree, resolves open issues, and records the removal", async () => {
    env.JOBS_TRIGGER_SECRET = JOBS_SECRET;
    env.SMPLKIT_API_KEY = "sk_api_test";
    const { token, user_id } = await register();
    const bm = await makeBenchmark(token);
    ...
    const res = await trigger({ benchmark_id: bm.id, reason: "PII removal", removed_by: "ops-1" }, JOBS_SECRET);
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ deleted: true, benchmark_id: bm.id, issues_resolved: 1 });
    ...
    // The issue is the surviving paper trail…
    const req = await env.DB.prepare("SELECT status, resolved_at FROM issue WHERE benchmark_id = ?")
      .bind(bm.id)
    ...
    await vi.waitFor(() =>
      expect(auditPosts.filter((e) => e.event_type === "benchmark.removed")).toHaveLength(1),
    );
    const ev = auditPosts.find((e) => e.event_type === "benchmark.removed");
    expect(ev?.resource_id).toBe(bm.id);
    expect(ev?.actor_type).toBe("OPERATOR");
    expect(ev?.actor_id).toBe("ops-1");
  ...
  it("no publisher-facing path reaches it: the benchmark DELETE stays 409 pointing at issues", async () => {
    const { token, user_id } = await register();
    const bm = await makeBenchmark(token);
    await publish(token, user_id, bm.id);
    ...
    expect(err.errors[0].detail).toContain("issue");
  });
});
