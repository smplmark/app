...
describe("benchmark_metric — snapshot on link", () => {
  it("links an INTEGER metric, appending a MetricDecl to the schema", async () => {
    ...
    // The unit + format snapshot into the schema so the display layer can render values, and the
    // metric's display label is resolved from the linked library metric on read.
    expect(schema.metrics).toEqual([
      { name: "throughput_req_s", label: "Throughput (req/s)", type: "INTEGER", unit: "req/s", format: "#,##0", description: "Requests per second" },
    ]);
  ...
  it("links a FORMULA metric, appending a DerivedDecl with the compiled expression", async () => {
    ...
    const metric = await makeMetric(me.token, {
      ...
      type: "FORMULA",
      unit: "ms",
      // Minute offset from primitives: created_at mod 60000.
      formula: {
    ...
    const schema = await schemaOf(me.token, bm.id);
    expect(schema.metrics).toEqual([]);
    // Linking surfaces the derived metric by name + display fields, plus the formula resolved live
    // from the library metric — the structured steps and the JSON Logic they compile to.
    expect(schema.derived).toEqual([
      {
        name: "offset",
        label: "Offset",
