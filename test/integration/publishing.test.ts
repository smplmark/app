// Publisher identity, domain verification, and the draft/publish workflow (draft → published).
...
describe("draft edit-lock", () => {
  async function readyChainWithData(): Promise<{ me: Registered; benchmark: Resource; subject: Resource; run: Resource }> {
    ...
    const ing = await apiPost(
      "/api/v1/measurements",
      { data: { type: "measurement", attributes: { run: run.id, subject: subject.id, metrics: { offset_ms: 1 } } } },
      bearer(me.token),
    );
  ...
  it("freezes the whole subtree while marked ready (PRIVATE && draft=0)", async () => {
    ...
    // benchmark edits
    expect(
      (await apiPut(`/api/v1/benchmarks/${benchmark.id}`, { data: { type: "benchmark", attributes: { name: "x", measurement_schema: OFFSET_SCHEMA } } }, tok)).status,
    ).toBe(409);
    ...
    // ingest
    expect(
      (await apiPost("/api/v1/measurements", { data: { type: "measurement", attributes: { run: run.id, subject: subject.id, metrics: { offset_ms: 2 } } } }, tok)).status,
    ).toBe(409);
...
// ── mark_ready / return_to_draft authority ─────────────────────────────────
...
describe("publish readiness gate", () => {
  ...
  it("names exactly the missing pieces when some are present", async () => {
    const me = await register();
    await markVerified(me.user_id);
    const b = await makeBenchmark(me.token); // OFFSET_SCHEMA → the metric leg is satisfied
  ...
  it("publishes once a subject, metric, run, and measurement all exist (200)", async () => {
    const me = await register();
    await markVerified(me.user_id);
    const b = await makeBenchmark(me.token); // OFFSET_SCHEMA supplies the metric
    const s = await makeSubject(me.token, b.id, "s1");
    const run = await makeRun(me.token, b.id);
    await makeMeasurement(me.token, run.id, s.id, { metrics: { offset_ms: 1 } });
    await markReady(me.token, b.id);
    await allowPersonalPublish(b.id);
...
describe("personal publish", () => {
  ...
  it("keeps ingest open after publishing; new measurements join the public record", async () => {
    ...
    await makeMeasurement(me.token, run.id, t.id, { metrics: { offset_ms: 1 } }); // clears the readiness gate
    await markReady(me.token, b.id);
    await allowPersonalPublish(b.id);
    ...
    const ing = await apiPost(
      "/api/v1/measurements",
      { data: { type: "measurement", attributes: { run: run.id, subject: t.id, metrics: { offset_ms: 5 } } } },
      bearer(me.token),
    );
    expect(ing.status).toBe(201);
...
// ── the never-retroactively-strip guarantee ─────────────────────────────
