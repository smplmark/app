...
describe("post-publish schema edits", () => {
  const put = (token: string, bench: Resource, schema: unknown) =>
    apiPut(
      `/api/v1/benchmarks/${bench.id}`,
      {
        data: {
          type: "benchmark",
          attributes: { name: "Example Bench", subject_type: bench.attributes.subject_type, measurement_schema: schema },
        },
      },
      bearer(token),
    );
  ...
  it("accepts appends, expression changes, chart changes, and removals after publish", async () => {
    ...
    // Appending a brand-new stored metric alongside the derived offset_ms lands — publishing no
    ...
    const grownDerived = {
      ...OFFSET_SCHEMA,
      derived: [
        ...OFFSET_SCHEMA.derived,
        { name: "offset_s", unit: "s", expr: { "/": [{ var: "offset_ms" }, 1000] } },
      ],
    };
    ...
    expect((await put(owner.token, bench, OFFSET_SCHEMA)).status).toBe(200);

    // Descriptions and unit labels inside existing entries remain editable.
    const relabeled = {
      ...OFFSET_SCHEMA,
      derived: [
        { ...OFFSET_SCHEMA.derived[0], unit: "milliseconds", description: "Milliseconds past the minute boundary." },
      ],
    };
    ...
    const mutated = {
      ...OFFSET_SCHEMA,
      derived: [{ name: "offset_ms", unit: "ms", expr: { var: "created_at" } }],
    };
    expect((await put(owner.token, bench, mutated)).status).toBe(200);

    // Changing the chart → 200.
    const rechart = { ...OFFSET_SCHEMA, chart: { x: null, y: "offset_ms", x_kind: "CATEGORY" } };
