...
// Each SPEC benchmark is flat: sibling subjects/runs/measurements. A measurement names one run + one
// subject; find the measurement for a given subject by joining on its (single) run.
type Bench = ReturnType<typeof adapt>[number];
const measurementForSubject = (b: Bench, subjectKey: string) => {
...
describe("spec source metadata", () => {
  it("attributes SPEC under its Fair Use Rules and caps at the platform subject limit", () => {
...
describe("spec adapt — crawl suite", () => {
  it("reads every quarter file listed in the manifest and cites the quarter-qualified result URL", () => {
    ...
    expect(power.subjects).toHaveLength(1);
    // 1:1 per independent result: one subject, one run, one measurement.
    expect(power.runs).toHaveLength(1);
    expect(power.measurements).toHaveLength(1);
    const t = power.subjects[0];
    expect(t.name).toBe("PowerEdge R7725 (Dell Inc.)");
    const meas = measurementForSubject(power, t.key);
...
describe("spec adapt", () => {
  ...
  it("maps CPU2017: base+peak, drops score-less rows, omits absent peak, cites the result page", () => {
    ...
    // 1:1 per independent result: run/subject/measurement counts match.
    ...
    const topMeas = measurementForSubject(c, top.key);
    ...
    const node = c.subjects.find((t) => t.name.startsWith("Node Two"))!;
    expect(measurementForSubject(c, node.key).metrics).toEqual({ base_score: 300 });
  ...
  it("maps SPECjbb2015: max-jOPS as the chart metric plus critical-jOPS", () => {
    ...
    const meas = measurementForSubject(j, j.subjects[0].key);
    expect(meas.metrics).toEqual({ max_jops: 250000, critical_jops: 200000 });
    expect(j.subjects[0].details).toMatchObject({ jvm: "Oracle Java SE 17" });
  ...
  it("honors the cap, keeping the highest-scoring slice", () => {
    ...
    expect(c.subjects[0].name).toBe("Box 9000 AMD EPYC 9754, 3.1GHz (ACME Corp)");
  });
});
