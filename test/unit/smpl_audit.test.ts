...
const OPERATOR: AuditEventInput["actor"] = { type: "OPERATOR", id: "ops-1", label: "ops-1" };
...
describe("emitAuditEvent", () => {
  ...
  it("POSTs the event envelope off the response path with the category correlation", async () => {
    ...
    const testEnv = { ...env, SMPLKIT_API_KEY: "sk_api_test", SMPLKIT_ENVIRONMENT: undefined } as unknown as Env;
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      return Response.json({
        data: [
          ...
          // An event of a type that has since been retired — dropped read-side.
