...
import { loadLiveDerivedByBenchmark, loadMetricLabelsByBenchmark } from "../../src/logic/live_derived";
import type { MeasurementSchema } from "../../src/types";

// created_at MOD 3600000 — ms elapsed in the current hour. Compiles to `{ "%": [{var:created_at}, 3600000] }`.
const ELAPSED_FORMULA = JSON.stringify({
...
describe("loadLiveDerivedByBenchmark", () => {
  ...
  it("resolves live FORMULA metrics — compiling expr, carrying cosmetic facets, skipping the rest", async () => {
    ...
    const elapsed = await createMetric(env.DB, { account_id: acc.id, name: "offset_ms", label: "Elapsed", description: "Elapsed in hour", type: "FORMULA", unit: "ms", format: "#,##0", formula: ELAPSED_FORMULA });
    ...
    // Result order is ORDER BY bm.created_at: elapsed (t=1) precedes doubled (t=2).
    await link(bench1, doubled.id, 2);
    await link(bench1, elapsed.id, 1);
    ...
    // Each decl carries both forms of the formula: the compiled JSON Logic `expr` the engine
    // evaluates, and the structured `formula` it compiled from (rendered on the benchmark page).
    expect(map.get(bench1)).toEqual([
      {
        name: "offset_ms", label: "Elapsed", expr: { "%": [{ var: "created_at" }, 3_600_000] }, formula: JSON.parse(ELAPSED_FORMULA),
        unit: "ms", format: "#,##0", description: "Elapsed in hour",
      },
      { name: "doubled", label: "Doubled", expr: { "*": [{ var: "metrics.x" }, 2] }, formula: JSON.parse(DOUBLE_FORMULA) },
    ]);
  });
  ...
  it("groups derived metrics by benchmark id", async () => {
    ...
    const m = await createMetric(env.DB, { account_id: acc.id, name: "offset_ms", label: "Elapsed", description: null, type: "FORMULA", unit: null, format: null, formula: ELAPSED_FORMULA });
...
describe("loadMetricLabelsByBenchmark", () => {
  it("returns an empty map for empty input (no query issued)", async () => {
    expect((await loadMetricLabelsByBenchmark(env.DB, [])).size).toBe(0);
  });

  it("maps every linked metric's name to its label — stored and FORMULA alike, grouped by benchmark", async () => {
    const acc = await createAccount(env.DB, { key: `acc-${crypto.randomUUID()}`, name: "Acc" });
    const st = await createSubjectType(env.DB, { account_id: acc.id, key: "st", name: "ST", fields: [] });
    const bench1 = await makeBenchmarkRow(acc.id, st.id, "bench-1");
    const bench2 = await makeBenchmarkRow(acc.id, st.id, "bench-2");
    ...
    const stored = await createMetric(env.DB, { account_id: acc.id, name: "total_ms", label: "Total Time", description: null, type: "INTEGER", unit: "ms", format: null, formula: null });
    const derived = await createMetric(env.DB, { account_id: acc.id, name: "offset_ms", label: "Offset", description: null, type: "FORMULA", unit: null, format: null, formula: ELAPSED_FORMULA });
    await link(bench1, stored.id, 1);
    await link(bench1, derived.id, 2);
    await link(bench2, stored.id, 1);

    const map = await loadMetricLabelsByBenchmark(env.DB, [bench1, bench2, benchEmpty]);
    expect(map.get(bench1)).toEqual({ total_ms: "Total Time", offset_ms: "Offset" });
    expect(map.get(bench2)).toEqual({ total_ms: "Total Time" });
    // A benchmark with no linked metric is absent, so the caller adds no labels at all.
    expect(map.has(benchEmpty)).toBe(false);
  });

  it("contributes nothing for a metric whose label is empty (the API forbids it; the DB doesn't)", async () => {
    const acc = await createAccount(env.DB, { key: `acc-${crypto.randomUUID()}`, name: "Acc" });
    const st = await createSubjectType(env.DB, { account_id: acc.id, key: "st", name: "ST", fields: [] });
    const bench = await makeBenchmarkRow(acc.id, st.id, "bench-1");
    const blank = await createMetric(env.DB, { account_id: acc.id, name: "offset_ms", label: "", description: null, type: "FORMULA", unit: null, format: null, formula: ELAPSED_FORMULA });
    await link(bench, blank.id, 1);

    expect((await loadMetricLabelsByBenchmark(env.DB, [bench])).has(bench)).toBe(false);
    // The derived decl is still resolved — just without a `label` key.
    expect(await loadLiveDerivedByBenchmark(env.DB, [bench]).then((m) => m.get(bench))).toEqual([
      { name: "offset_ms", expr: { "%": [{ var: "created_at" }, 3_600_000] }, formula: JSON.parse(ELAPSED_FORMULA) },
    ]);
  });
});
