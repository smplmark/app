...
describe("summarizeMeasurements", () => {
  // A schema with one stored metric (latency_ms) and one derived metric (offset_ms = created_at mod 60000).
  const SCHEMA = JSON.stringify({
    metrics: [{ name: "latency_ms", type: "DECIMAL" }],
    derived: [{ name: "offset_ms", unit: "ms", expr: { minute_offset_ms: [{ var: "created_at" }] } }],
  });
  const TOP = Date.UTC(2026, 6, 1, 10, 0, 0); // top-of-minute → the derived value is the +offset
  ...
  it("computes derived metrics from the LIVE map, not the stored snapshot", () => {
    // The live map replaces the snapshot's derived (offset_ms) with a differently-named derived metric,
    ...
    const live = new Map<string, import("../../src/types").DerivedDecl[]>([
      ["bench-1", [{ name: "live_offset", unit: "ms", expr: { minute_offset_ms: [{ var: "created_at" }] } }]],
    ]);
    const out = summarizeMeasurements([row("sub-a", 250, 10)], false, live);
    const m = out.subjects[0].metrics;
    expect(Object.keys(m).sort()).toEqual(["latency_ms", "live_offset"]); // snapshot's offset_ms is gone
    expect(m.live_offset).toMatchObject({ count: 1, avg: 250 });
  });
