...
describe("validateMeasurementSchema", () => {
  it("normalizes a full valid schema", () => {
    const out = validateMeasurementSchema({
      ...
      derived: [
        { name: "offset_ms", unit: "ms", expr: { minute_offset_ms: [{ var: "created_at" }] } },
      ],
    });
    expect(out).toEqual({
      metrics: [{ name: "p95_ms", type: "number", unit: "ms" }],
...
describe("parseMeasurementSchema", () => {
  ...
  it("round-trips a chart block", () => {
    const json = JSON.stringify({
      metrics: [{ name: "offset_ms", type: "number" }],
      derived: [],
      chart: { x: "created_at", y: "offset_ms", x_kind: "TIME" },
    });
    expect(parseMeasurementSchema(json).chart).toEqual({ x: "created_at", y: "offset_ms", x_kind: "TIME" });
  });
});
...
describe("chart validation", () => {
  ...
  it("accepts a time-series chart and a scalar (x=null) chart", () => {
    expect(validateMeasurementSchema(withMetric({ x: "created_at", y: "offset_ms", x_kind: "TIME" })).chart)
      .toEqual({ x: "created_at", y: "offset_ms", x_kind: "TIME" });
    expect(validateMeasurementSchema(withMetric({ x: null, y: "offset_ms" })).chart)
      .toEqual({ x: null, y: "offset_ms" });
  });
  ...
  it.each([
    [withMetric({ y: "offset_ms", x: "nope" }), "unknown x metric"],
    [withMetric({ y: "nope" }), "unknown y metric"],
    [withMetric({ x: "created_at" }), "missing y"],
    [withMetric({ x: "created_at", y: "offset_ms", x_kind: "PIE" }), "bad x_kind"],
  ])("rejects %o (%s)", (value, _label) => {
    expect400(() => validateMeasurementSchema(value));
  });
});
...
describe("diffMeasurementSchema", () => {
  const published: MeasurementSchema = {
    metrics: [{ name: "offset_ms", type: "number", unit: "ms", description: "old" }],
  ...
  it("ignores key-order-only differences (canonical compare)", () => {
    const reordered: MeasurementSchema = {
      metrics: [{ type: "number", description: "old", unit: "ms", name: "offset_ms" }],
      derived: [{ expr: { minute_offset_ms: [{ var: "created_at" }] }, unit: "ms", name: "d" }],
      chart: { y: "offset_ms", x_kind: "TIME", x: "created_at" },
    };
  ...
  it("flags cosmetic unit/format/description edits as changed, but not semantic-core", () => {
    const edited: MeasurementSchema = {
      metrics: [
        { name: "offset_ms", type: "number", unit: "milliseconds", format: "#,##0", description: "new" },
      ],
      derived: [{ name: "d", expr: { minute_offset_ms: [{ var: "created_at" }] }, unit: "ms" }],
      chart: { x: "created_at", y: "offset_ms", x_kind: "TIME" },
    };
  ...
  it("flags chart changes as semantic-core: added, changed, and removed", () => {
    ...
    // Changed mapping.
    expect(
      diffMeasurementSchema(published, { ...published, chart: { x: null, y: "offset_ms" } }),
