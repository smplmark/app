...
const OFFSET_SCHEMA: MeasurementSchema = {
  metrics: [],
  derived: [
...
describe("computeMetrics", () => {
  it("computes a derived metric for a bare (null-metrics) measurement", () => {
    expect(computeMetrics(null, OFFSET_SCHEMA, ctx(MS))).toEqual({ offset_ms: 87 });
  });

  it("merges stored metrics with derived metrics", () => {
    expect(computeMetrics('{"p95_ms":12.4}', OFFSET_SCHEMA, ctx(MS))).toEqual({
      p95_ms: 12.4,
      offset_ms: 87,
    });
  });
  ...
  it("treats invalid / non-object stored JSON as empty", () => {
    expect(computeMetrics("not json", OFFSET_SCHEMA, ctx(MS))).toEqual({ offset_ms: 87 });
  ...
  it("lets a derived value win a name collision with a stored value", () => {
    expect(computeMetrics('{"offset_ms":999}', OFFSET_SCHEMA, ctx(MS))).toEqual({ offset_ms: 87 });
  });
