  ...
  it("substitutes live derived metrics for the stored snapshot when provided", () => {
    ...
    const row: BenchmarkRowWithPublisher = {
      ...
      measurement_schema: JSON.stringify({
        ...
        derived: [{ name: "offset_ms", unit: "ms", expr: { "%": [{ var: "created_at" }, 60000] } }],
        chart: { x: "created_at", y: "offset_ms" },
      }),
    };
    const liveFormula = {
      steps: [{ id: "A", kind: "OP" as const, op: "MOD" as const, a: { kind: "CREATED_AT" as const }, b: { kind: "NUMBER" as const, value: 3_600_000 } }],
      result: "A",
    };
    const live = [
      {
        name: "offset_ms", unit: "s", format: "0.0", description: "seconds past the hour",
        formula: liveFormula, expr: { "%": [{ var: "created_at" }, 3_600_000] },
      },
    ];
    const out = serializeBenchmark(row, [], live);
    expect(out.attributes.measurement_schema).toEqual({
      metrics: [{ name: "latency_ms", type: "DECIMAL" }],
      // The formula IS surfaced, in both forms — the structured steps and the JSON Logic they compile
      // to — so a visitor can see exactly how the derived value is computed.
      derived: [{
        name: "offset_ms", unit: "s", format: "0.0", description: "seconds past the hour",
        formula: liveFormula, expr: { "%": [{ var: "created_at" }, 3_600_000] },
      }],
    ...
    // With no liveDerived, the stored snapshot is surfaced — its recorded `expr` and no structured
    // formula (a snapshot never carried one).
    expect(serializeBenchmark(row, []).attributes.measurement_schema).toMatchObject({
      derived: [{ name: "offset_ms", unit: "ms", expr: { "%": [{ var: "created_at" }, 60000] } }],
    });
  });

  it("folds each linked metric's display label into the schema (stored and derived alike)", () => {
    const row: BenchmarkRowWithPublisher = {
      ...priv,
      measurement_schema: JSON.stringify({
        metrics: [{ name: "latency_ms", type: "DECIMAL" }, { name: "unlinked", type: "INTEGER" }],
        derived: [{ name: "offset_ms", unit: "ms" }],
      }),
    };
    const labels = { latency_ms: "Latency", offset_ms: "Offset" };
    expect(serializeBenchmark(row, [], undefined, labels).attributes.measurement_schema).toEqual({
      metrics: [{ name: "latency_ms", label: "Latency", type: "DECIMAL" }, { name: "unlinked", type: "INTEGER" }],
      derived: [{ name: "offset_ms", label: "Offset", unit: "ms" }],
    });
    // A live decl carries its own label, which wins over the map.
    const live = [{ name: "offset_ms", label: "Offset (live)", unit: "s" }];
    expect(serializeBenchmark(row, [], live, labels).attributes.measurement_schema).toMatchObject({
      derived: [{ name: "offset_ms", label: "Offset (live)", unit: "s" }],
    });
    // No labels passed → nothing is added.
    expect(serializeBenchmark(row, []).attributes.measurement_schema).toMatchObject({
      metrics: [{ name: "latency_ms", type: "DECIMAL" }, { name: "unlinked", type: "INTEGER" }],
    });
  });
...
describe("serializeSubject", () => {
  const row: SubjectRow & { subject_type_key: string | null } = {
    id: "t1", account_id: "a1", subject_type_id: "st1", subject_type_key: "server", key: "server-a", name: "Server A",
  ...
  it("uses the subject's key as its public id (the internal UUID is never surfaced)", () => {
    ...
    expect(out.id).toBe("server-a");
    expect(out.id).toBe(row.key);
    expect(out.attributes.key).toBe("server-a"); // the key attribute is retained; it equals the id
  });
});
...
describe("serializeRun", () => {
  const base: RunRow = {
    ...
    invalidated_by_user_id: null, measurement_count: 0, created_at: T0, updated_at: T0,
  };
...
describe("serializeMeasurement", () => {
  const schema: MeasurementSchema = {
    metrics: [],
    derived: [{ name: "offset_ms", expr: { minute_offset_ms: [{ var: "created_at" }] } }],
  };
  ...
  const base: Pick<MeasurementRow, "id" | "run_id" | "subject_id" | "created_at" | "metrics" | "meta"> & {
    ...
    id: 48213, run_id: "r1", subject_id: "tg1", subject_key: "server-a", run_key: "default", created_at: T0 + 87, metrics: null, meta: null,
  };
  ...
  it("computes derived metrics, stringifies id, emits the run and subject by their keys (not UUIDs)", () => {
    ...
    expect(out.attributes).toEqual({
      created_at: "2026-07-01T09:00:00.087Z", run: "default", subject: "server-a", metrics: { offset_ms: 87 },
    });
  });
...
describe("serializeIssue", () => {
  it("echoes the reported issue with an ISO created_at", () => {
    const out = serializeIssue({
