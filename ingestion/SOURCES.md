...
| Mapping | Two benchmarks mirroring the site's headline split: `blender-cpu` and `blender-gpu` (category `HARDWARE`; tags `rendering`, `blender`, `cycles`, + `cpu`/`gpu`). Subject = device; run = version slice (`v5.1.1`, GPU also per API `v5.1.1-optix`); measurement metrics `median_score` (samples/min, higher is better) + `submission_count`. Scalar → CATEGORY chart on `median_score`. |
...
| Mapping | One benchmark `helm-capabilities` (category `ML_AI`; tags `llm`, `evaluation`, `helm`). Subject = model (meta: creator org, access, release date); run = release (`v1.15.0`); measurement metrics: `mean_score`, `mmlu_pro`, `gpqa`, `ifeval`, `wildbench`, `omni_math` (0–1, higher is better). Scalar → CATEGORY chart on `mean_score`. |
...
| Mapping | One benchmark `open-llm-leaderboard` (category `ML_AI`; tags `llm`, `evaluation`, `open-weights`, `huggingface`). Subject = evaluated model config (`eval_name`; meta: precision, architecture, params, hub license); one run per subject dated by `Submission Date`; measurement metrics: `average`, `ifeval`, `bbh`, `math_lvl_5`, `gpqa`, `musr`, `mmlu_pro` (normalized 0–100, higher is better). Scalar → CATEGORY chart on `average`. Rows with `Flagged == true` are skipped (mirrors the upstream default view). |
...
| Mapping | One benchmark per CC18 task (`openml-cc18-<data_name>`, category `ML_AI`; tags `openml`, `cc18`, `classification`) — subject = flow (best run per flow after dedupe), one run per subject (provenance: run id, upload time), metric `predictive_accuracy`. Plus one benchmark `openml-amlb` for study 226 (subject = AutoML framework, run per task). Scalar → CATEGORY charts. |
...
| Mapping | One benchmark `clickbench` (category `DATABASE`; tags `olap`, `sql`, `analytics`). Subject = system + machine; run = the dated benchmark execution; measurement metrics: `load_time_s`, `data_size_bytes`, `cold_total_s`, `hot_total_s` (sums over the 43 queries; the official "relative" score needs a cross-subject baseline and is deliberately not reproduced); meta keeps the 43×3 per-query matrix. Scalar → CATEGORY chart on `hot_total_s`. |
| Verified | 2026-07-04 |
...
| | |
|---|---|
| What | Audited, compliant SPEC results (www.spec.org): SPEC CPU 2017 (Integer/Floating-Point × Rate/Speed — four benchmarks), SPECjbb 2015 (Java server throughput), SPEC HPC 2021, SPECpower_ssj2008 (server energy efficiency), and SPECstorage Solution 2020 (storage). Eight benchmarks. |
...
| Pull | **Aggregate-page suites** — one result-listing HTML page each (the four CPU2017 metric pages `cpu2017/results/{rint,rfp,cint,cfp}2017.html` ~10 MB each, `jbb2015/results/jbb2015.html`, `hpc2021/results/hpc2021.html`) — every result's headline score is inline, no per-result crawl. **Crawl suites** — `power_ssj2008` and `storage2020` publish no single all-results page, so the pull fetches the results landing, enumerates its `resYYYYqN` quarter directories, and fetches each quarter's listing page (whose result links are bare, no quarter prefix). Every request honors `Crawl-delay: 10`; power spans ~70 quarters (2007→now). |
| Mapping | One benchmark per page/suite. Subject = a tested system (deduped by sponsor + system + SPEC result id); one completed, audited run per result. Metrics: base + peak SPEC score (CPU2017 / HPC2021, `HARDWARE`), max-jOPS + critical-jOPS (jbb2015, `HARDWARE`), overall ssj_ops/watt (power, `HARDWARE`), or business metric + overall response time (storage, `STORAGE`; the workload/NAS type is a detail — results compare only within a workload). System config (cores/chips/JVM/nodes/workload) kept in details; `details.vendor` enriched for the leaderboard facet. Each measurement's `source_url` is the SPEC result page. Scalar → CATEGORY chart on the primary metric. |
| Default import cap | Top 500 results per benchmark by score; `--full` raises to the platform max of **20,000 subjects/benchmark** (`src/limits.ts`, raised for CPU2017's ~11.8k results/metric), so `--full` now keeps the whole corpus. |
| Verified | 2026-07-05 |
...
- **OpenBenchmarking.org** — "all rights reserved", no reuse license, no API, active bot-detection.
  Fails the "no hint it's not okay" test; excluded.
