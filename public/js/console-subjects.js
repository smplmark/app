...
// Subjects (/account/subjects) — a two-pane workspace: a narrow
...
(function () {
  ...
  async function boot() {
    try {
      const doc = await apiFetchAll("/api/v1/subject_types");
  ...
  async function loadSubjects(t) {
    try {
      SUBJECTS = [];
      const doc = await apiFetchAll("/api/v1/subjects?filter[subject_type]=" + encodeURIComponent(t.id));
      SUBJECTS = (doc && doc.data) || [];
