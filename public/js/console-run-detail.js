...
(function () {
  ...
  async function loadMeasurements() {
    ...
    try {
      ...
      const [measDoc, subjDoc] = await Promise.all([
        apiFetchAll("/api/v1/measurements?filter[run]=" + encodeURIComponent(ID)),
        benchId ? apiFetchAll("/api/v1/subjects?filter[benchmark]=" + encodeURIComponent(benchId)) : Promise.resolve(null),
      ]);
  ...
  //    stored and derived alike (e.g. latency_ms). The Range control narrows the set via a server-side
  ...
  async function renderApiReference() {
    ...
    // subject fields show keys (e.g. "server-a"), never UUIDs.
    let subjects = [];
    try {
      const benchId = (RUN.attributes || {}).benchmark;
      const sd = benchId ? await apiFetchAll("/api/v1/subjects?filter[benchmark]=" + encodeURIComponent(benchId)) : null;
      subjects = (sd && sd.data) || [];
