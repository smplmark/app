...
(function () {
  ...
  function headerActions() {
    ...
    if (status === "PUBLISHED") {
      return viewLink(a.key) + (a.closed ? b("Reopen", "reopen") : b("Close", "close")) + b("Report issue", "issue") + b("Withdraw", "withdraw", "Danger");
    }
    // WITHDRAWN: the record stays public; the remaining affordance is asking operators to remove it.
    return viewLink(a.key) + b("Report issue", "issue");
  }
  ...
  async function renderSubjects(panel, actions) {
    ...
    try {
      const [linkedDoc, linksDoc] = await Promise.all([
        ...
        apiFetchAll("/api/v1/benchmark_subjects?filter[benchmark]=" + encodeURIComponent(ID)),
      ]);
  ...
  async function renderRuns(panel, actions) {
    ...
    try {
      const doc = await apiFetchAll("/api/v1/runs?filter[benchmark]=" + encodeURIComponent(ID));
      RUNS = (doc && doc.data) || [];
      table.setRows(RUNS);
  ...
  function eventLabel(t) {
    const labels = {
      ...
      "benchmark.removed": "Removed by operators",
      "benchmark.issue_reported": "Issue reported",
  ...
  // ── Report-issue modal — files an issue for OPERATOR review; it never deletes anything. ──
  function openIssueModal() {
    const a = BM.attributes || {};
    const bodyHtml =
      '<form class="form" id="issue-form" novalidate>' +
      ...
      '<label class="field"><span class="detailFieldLabel fieldRequired">Why should it be removed?</span><textarea name="tdetails" rows="4" placeholder="e.g. it contains personal data that must be removed"></textarea><p class="fieldErrorMessage" hidden></p></label>' +
      '<p class="form-status" id="issue-msg"></p>' +
      '<div class="modalActions"><button type="button" class="button buttonSecondary buttonSmall" data-close>Cancel</button>' +
      '<button type="submit" class="button buttonPrimary buttonSmall">Send report</button></div></form>';
    const m = SM.modal({
      title: "Report an issue",
      description: "Asks smplmark operators to remove “" + (a.name || a.key || "") + "” entirely. This reports an issue for review — nothing is deleted until an operator acts on it. To retract the benchmark yourself while keeping the record, use Withdraw instead.",
      bodyHtml: bodyHtml,
      width: 560,
    });
    const f = m.panel.querySelector("#issue-form");
    const msg = m.panel.querySelector("#issue-msg");
    f.addEventListener("submit", async (ev) => {
      ev.preventDefault();
      msg.textContent = ""; msg.className = "form-status";
      let ok = true;
      [["tname", "Your name is required."], ["temail", "Your email is required."], ["tdetails", "A reason is required."]].forEach(([name, err]) => {
      ...
      const submit = f.querySelector('button[type="submit"]'); submit.disabled = true;
      try {
        await apiFetch("/api/v1/issues", { method: "POST", body: jsonapiBody("issue", {
          benchmark: ID,
          requester_name: f.tname.value.trim(),
          requester_email: f.temail.value.trim(),
          reason: f.tdetails.value.trim(),
        }) });
        m.close();
        SM.toast("Issue reported — smplmark operators will review it.", { kind: "success" });
      } catch (err) { submit.disabled = false; msg.textContent = err.message; msg.className = "form-status is-error"; }
    });
  }
  ...
  async function lifecycle(act) {
    ...
    if (act === "issue") { openIssueModal(); return; }
  ...
  async function loadVerifiedPublishers() {
    const doc = await apiFetchAll("/api/v1/publishers?filter[status]=VERIFIED", null);
